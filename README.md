...
1. **User Onboarding UI** – Accepts user details for hospital enlistment.
2. **Google Sheets Integration** – Uses Pica to push form submissions into a Google Sheet.
3. **Welcome Email Service** – Initially used Gmail API via Pica, but was blocked by Google.
4. **PostgreSQL Integration** – Stores user data in a local Postgres database as a fallback.
5. **GitHub & OpenAI Integration** – 
   - Automatically creates a new GitHub branch per user using Pica.
   - Prepares user data for further processing or AI-driven insights (via OpenAI).
6. **LLM Integration (Experimental)** – 
   - Used local **LLaMA 3 (Ollama)** and **Groq APIs** for additional AI tasks.
   - Test files related to this integration are also included in this repo.
---
